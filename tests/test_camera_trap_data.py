...
def test_get_camera_trap_data_returns_image_and_video_without_duplicate_columns(hdr):
    ...
    def fake_get_media_assets_df(_hdr, datatype, project_system_record_ids=None, **_kwargs):
    ...
    assert result["label"].tolist() == ["label-1", "label-2"]
...
def test_get_camera_trap_data_keeps_partial_results_when_one_datatype_fails(hdr):
    image_stations = _station_frame(101, "image", 12.1, -0.1)
    video_stations = _station_frame(202, "video", 12.2, -0.2)
    all_camera_stations = pd.concat([image_stations, video_stations], ignore_index=True)
...
def test_get_camera_trap_data_drops_unlabelled_and_blank_rows(hdr):
    ...
    unlabelled = _media_record(101, 2).model_dump(mode="json")
    unlabelled["label"] = None
    unlabelled["label_id"] = None
    blank = _media_record(101, 3).model_dump(mode="json")
    blank["blank"] = True

    with (
        patch("naturecubepy.api.get_station_info", return_value=stations),
        patch(
            "naturecubepy.api.get_media_assets_df",
            return_value=pd.DataFrame([labelled, unlabelled, blank]),
        ),
    ):
        result = get_camera_trap_data(hdr)
